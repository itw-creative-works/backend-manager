/**
 * Test: admin:firestore-query
 * Tests the admin Firestore query command
  ...
  tests: [
    // Test 1: Setup - write test documents
    {
      ...
      async run({ http, assert, state }) {
        // Write multiple test documents to query
        state.testDocs = [
        ...
        for (let i = 0; i < state.testDocs.length; i++) {
          ...
          const response = await http.as('admin').command('admin:firestore-write', {
    ...
    // Test 2: Query all documents in collection
    {
      ...
      async run({ http, assert, state }) {
        const queryResponse = await http.as('admin').command('admin:firestore-query', {
          queries: [
    ...
    // Test 8: Unauthenticated request fails
    {
      ...
      async run({ http, assert }) {
        const queryResponse = await http.as('none').command('admin:firestore-query', {
